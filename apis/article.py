...
async def toggle_article_featured_status(
    article_id: str,
    is_featured: bool = Query(..., description="精选状态: true为精选, false为取消精选"),
    current_user: dict = Depends(get_current_user_or_ak)
):
    session = DB.get_session()
    try:
        ...
        article.is_featured = 1 if is_featured else 0
        session.commit()
        ...
        return success_response({
            "message": "文章已加入精选" if is_featured else "已取消精选",
