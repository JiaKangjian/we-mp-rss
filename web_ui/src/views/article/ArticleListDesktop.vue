        ...
        <a-card style="border:0">
          ...
          <div class="search-bar">
            ...
            <a-select v-model="articleFilterType" class="article-filter-select" @change="handleArticleFilterChange" size="small" :style="{ width: '100px' }" placeholder="筛选">
              ...
              <a-option value="featured">精选</a-option>
          ...
          <a-table :columns="columns" :data="articles" :loading="loading" :pagination="pagination"
            ...
            <template #actions="{ record }">
              <a-space>
                ...
                <a-button type="text" @click="toggleFeaturedStatus(record)" :title="record.is_featured === 1 ? '取消精选' : '精选'" :style="record.is_featured === 1 ? 'color: rgb(var(--warning-6))' : ''">
                  <template #icon>
                    <icon-thumb-up-fill v-if="record.is_featured === 1" />
...
import { getArticles, deleteArticle as deleteArticleApi, ClearArticle, ClearDuplicateArticle, getArticleDetail, getRefreshArticleTaskStatus, refreshArticle as refreshArticleApi, toggleArticleFavoriteStatus, toggleArticleFeaturedStatus, toggleArticleReadStatus, cleanOldArticles } from '@/api/article'
...
const fetchArticles = async () => {
  ...
  try {
    ...
    // 点击精选文章时用 only_featured 替代 mp_id 过滤
    if (activeMpId.value === FEATURED_MP_ID) {
      params.only_featured = true
    } else {
      params.mp_id = activeMpId.value
    }
    ...
    articles.value = (res.list || []).map(item => ({
      ...
      is_featured: item.is_featured === 1 ? 1 : 0
    }))
...
const toggleFeaturedStatus = async (record: any) => {
  try {
    const newFeaturedStatus = record.is_featured === 1 ? false : true
    await toggleArticleFeaturedStatus(record.id, newFeaturedStatus)

    const index = articles.value.findIndex(item => item.id === record.id)
    if (index !== -1) {
      articles.value[index].is_featured = newFeaturedStatus ? 1 : 0
    }

    Message.success(newFeaturedStatus ? '已加入精选' : '已取消精选')
  } catch (error) {
    console.error('更新精选状态失败:', error)
    Message.error('更新精选状态失败')
  }
}
</script>
